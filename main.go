...
func login(c *gin.Context, db *sql.DB) {
	...
	row := db.QueryRow("SELECT id FROM users WHERE username = ? AND password = ?",
		user.Username, user.Password)
	...
	if err != nil {
		...
		if err == sql.ErrNoRows {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalidad username or password"})
		}
...
func createChannel(c *gin.Context, db *sql.DB) {
	...
	result, err := db.Exec("INSERT INTO channels (name) VALUES (?)",
		channel.Name)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	// Return ID of newly inserter channel
	c.JSON(http.StatusOK, gin.H{"id": id})
}
...
func listMessages(c *gin.Context, db *sql.DB) {
	...
	rows, err := db.Query("SELECT m.id, channel_id, user_id, u.username AS user_name, message FROM messages m LEFT JOIN users u ON u.id = m.user_id WHERE channel_id = ? AND m.id > ? ORDER BY m.id ASCT LIMIT ?", channelID, lastMessageID, limit)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(&message.ID, &message.ChannelID, &message.UserName, &message.Text)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
